...
const Card = ({ classes, name, isActive, onClick }) => {
    return (
        <div role="presentation" className={classes.root} onClick={onClick}>
            <div className={classNames(classes.flipper, { [classes.flipperRotate]: isActive })} >
                <MCard className={classNames(classes.page, classes.pageFront)} />
                <MCard className={classNames(classes.page, classes.pageBack)} >
                    <CardContent><Typography variant="h5" component="h2">{ name }</Typography></CardContent>
                </MCard>
            </div>
        </div>
    )
